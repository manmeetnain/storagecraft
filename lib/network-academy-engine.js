...
function initialState(profile){
  ...
  if(profile==='cisco')return{hostname:'MDS-A',mode:'exec',context:null,vsans:{1:{name:'default'}},interfaces:Object.fromEntries(Array.from({length:8},(_,i)=>[`fc1/${i+1}`,{enabled:true,speed:'auto',vsan:1,state:i<2?'up':'down',mode:'F',pwwn:i===0?'10:00:00:00:00:00:00:01':i===1?'50:00:00:00:00:00:00:01':null,fcid:i===0?'0x010100':i===1?'0x010200':null}])),deviceAliases:{},zones:{},zonesets:{},activeZonesets:{},logs:[],startupConfig:null,saved:false};
  return{hostname:'iscsi-lab',vlans:{},interfaces:{eth0:{ip:'192.0.2.10/24',vlan:null,up:true}},targets:{},initiators:{host1:{iqn:'iqn.2026-08.lab:host1',discoveries:[],sessions:[]}},multipathPolicy:'round-robin',failedPaths:[],saved:false};
}
...
export function evaluateAcademyScenario(session){
  if(!session.scenario)return{active:false,passed:false,score:0,output:'No scenario is active. Use scenario list and scenario load <id>.'};const scenario=ACADEMY_SCENARIOS[session.profile].find(item=>item.id===session.scenario.id),passed=scenario.check(session.state);session.scenario.checks++;session.scenario.complete=passed;const score=passed?Math.max(60,100-Math.max(0,session.scenario.commands-2)*5):0;if(passed&&(session.scenario.bestScore===null||score>session.scenario.bestScore))session.scenario.bestScore=score;if(passed){const prior=session.assessments[scenario.id];if(!prior||score>prior.bestScore)session.assessments[scenario.id]={title:scenario.title,bestScore:score,bestCommands:session.scenario.commands,completed:true}}return{active:true,passed,score,title:scenario.title,hint:scenario.hint,commands:session.scenario.commands,output:passed?`PASS · ${scenario.title}\nScore: ${score}/100 · Commands: ${session.scenario.commands}`:`NOT YET RESOLVED · ${scenario.title}\nHint: ${scenario.hint}`};
}

export function academyReport(session){const total=ACADEMY_SCENARIOS[session.profile].length,completed=Object.values(session.assessments).filter(x=>x.completed),average=completed.length?Math.round(completed.reduce((sum,x)=>sum+x.bestScore,0)/completed.length):0;return{profile:session.profile,profileName:ACADEMY_PROFILES[session.profile].name,completed:completed.length,total,completionPercent:Math.round(completed.length/total*100),averageScore:average,commandsRun:session.commandHistory.length,assessments:clone(session.assessments)};}
...
export function createAcademySession(profile='brocade'){
  ...
  return{profile,state:initialState(profile),undoStack:[],commandHistory:[],assessments:{}};
}
...
function help(profile){
  ...
  if(profile==='cisco')return`${common}\nshow: show version|vsan|interface [brief|fc1/1]|flogi database|fcns database|logging|running-config|startup-config|device-alias database|zone|zoneset [active]\nmode: configure terminal, exit, end\nconfig: hostname, vsan <id> [name <name>], interface fc1/1, device-alias database, zone name <name> vsan <id>, zoneset name <name> vsan <id>\ninterface: shutdown|no shutdown, switchport speed <value>, switchport vsan <id>, switchport mode <F|E|NP>\nremove: use the no form; activate: zoneset activate name <name> vsan <id>\nsave: copy running-config startup-config`;
  return`${common}\nnetwork: vlan create|delete, interface set|delete|up|down\nstorage: target create|delete, portal add|delete, lun map|unmap, chap set|clear\nhost: discover, login, logout, sessions\nmultipath: multipath show, multipath policy <round-robin|failover>, path fail|restore`;
}

const HELP_TOPICS={
  brocade:{alicreate:'alicreate <alias>, <pwwn>[; <pwwn>]\nCreate a symbolic alias. Members must be valid pWWNs.',zonecreate:'zonecreate <zone>, <alias-or-pwwn>[; ...]\nCreate a zone from defined aliases or pWWNs.',cfgcreate:'cfgcreate <config>, <zone>[; ...]\nCreate a defined configuration from existing zones.',cfgenable:'cfgenable <config>\nValidate and make a defined configuration effective.',porterrshow:'porterrshow\nDisplay per-port CRC, encoding, and link-failure counters.',portcfgspeed:'portcfgspeed <port> <auto|4|8|16|32|64>\nSet the simulated FC link speed.'},
  cisco:{vsan:'vsan <1-4093> [name <name>]\nCreate or name a VSAN in configuration mode. Use no vsan <id> to remove an unreferenced VSAN.',interface:'interface fc1/<port>\nEnter FC interface configuration mode.','device-alias':'device-alias database\ndevice-alias name <name> pwwn <wwn>\ndevice-alias commit\nCreate and commit persistent device aliases.',zone:'zone name <name> vsan <id>\nEnter zone mode, then add member device-alias <name> or member pwwn <wwn>.',zoneset:'zoneset name <name> vsan <id>\nAdd member <zone>, exit, then zoneset activate name <name> vsan <id>.',flogi:'show flogi database\nDisplay simulated fabric logins by interface and VSAN.'},
  iscsi:{target:'target create|delete <iqn>\nCreate a target or delete one after every active session logs out.',portal:'portal add|delete <iqn> <address:port>\nManage target portal endpoints.',lun:'lun map <iqn> <lun> <backing-store>\nlun unmap <iqn> <lun>\nManage target LUN mappings.',chap:'chap set <iqn> <user> <secret>\nchap clear <iqn>\nConfigure or remove simulated one-way CHAP.',login:'discover\nlogin <iqn> [user secret]\nlogout <iqn>\nDiscovery and an active portal are required before login.',multipath:'multipath show\nmultipath policy <round-robin|failover>\nInspect paths and select the simulated policy.'}
};

function contextualHelp(profile,query){const topics=HELP_TOPICS[profile],key=Object.keys(topics).find(name=>query===name||query.startsWith(`${name} `)||name.startsWith(query));return key?topics[key]:`No focused help for '${query}'.\n\n${help(profile)}`;}
...
function cisco(session,line){
  ...
  if(lower.startsWith('show interface fc')){const id=t[2]?.toLowerCase(),i=s.interfaces[id];return i?ok(`${id} is ${i.enabled?i.state:'administratively down'}\n  Port mode is ${i.mode||'F'}\n  Port vsan is ${i.vsan}\n  Speed is ${i.speed}\n  ${i.pwwn?`Attached pWWN ${i.pwwn}`:'No fabric login'}`):bad(`Unknown interface ${id}`)}
  if(lower==='show flogi database')return ok(`INTERFACE  VSAN  FCID      PORT NAME\n${Object.entries(s.interfaces).filter(([,i])=>i.enabled&&i.pwwn).map(([id,i])=>`${id.padEnd(10)} ${String(i.vsan).padEnd(5)} ${i.fcid||'0x010000'} ${i.pwwn}`).join('\n')||'No FLOGI entries'}`);
  if(lower==='show fcns database')return ok(`VSAN  FCID      TYPE  PWWN\n${Object.values(s.interfaces).filter(i=>i.enabled&&i.pwwn).map(i=>`${String(i.vsan).padEnd(5)} ${i.fcid||'0x010000'} N     ${i.pwwn}`).join('\n')||'No FCNS entries'}`);
  if(lower==='show logging')return ok((s.logs||[]).join('\n')||'No simulated events');
  ...
  if(lower==='show startup-config')return ok(s.startupConfig?JSON.stringify(s.startupConfig,null,2):'Startup configuration is empty');
  if(lower==='copy running-config startup-config'){s.saved=true;s.startupConfig=clone({...s,mode:'exec',context:null,startupConfig:undefined});s.logs=(s.logs||[]).concat('%SYS-5-CONFIG_I: Configured from console and saved');return ok('Copy complete.',true)}
  ...
  if(s.mode==='config'&&cmd==='vsan'){const id=Number(t[1]);if(!Number.isInteger(id)||id<1||id>4093)return bad('VSAN must be 1-4093');const nameIndex=t.findIndex(x=>x.toLowerCase()==='name');s.vsans[id]=s.vsans[id]||{name:`vsan${id}`};if(nameIndex>=0)s.vsans[id].name=need(t[nameIndex+1],'VSAN name');s.logs=(s.logs||[]).concat(`%VSAN-5-CREATED: VSAN ${id} ${s.vsans[id].name}`);return ok('',true)}
  if(s.mode==='config'&&lower.startsWith('no vsan ')){const id=Number(t[2]);if(id===1)return bad('VSAN 1 cannot be removed');if(Object.values(s.interfaces).some(i=>i.vsan===id))return bad(`VSAN ${id} is assigned to an interface`);if(Object.values(s.zones).some(z=>z.vsan===id)||Object.values(s.zonesets).some(z=>z.vsan===id))return bad(`VSAN ${id} is referenced by zoning configuration`);delete s.vsans[id];return ok('',true)}
  ...
  if(s.mode==='interface'){const i=s.interfaces[s.context];if(lower==='shutdown'){i.enabled=false;i.state='down';s.logs=(s.logs||[]).concat(`%PORT-5-IF_DOWN_ADMIN_DOWN: ${s.context}`);return ok('',true)}if(lower==='no shutdown'){i.enabled=true;i.state=s.context==='fc1/1'||s.context==='fc1/2'?'up':'down';s.logs=(s.logs||[]).concat(`%PORT-5-IF_UP: ${s.context}`);return ok('',true)}if(lower.startsWith('switchport speed ')){if(!['auto','4000','8000','16000','32000','64000'].includes(t[2]))return bad('Speed must be auto, 4000, 8000, 16000, 32000, or 64000');i.speed=t[2];return ok('',true)}if(lower.startsWith('switchport vsan ')){const id=Number(t[2]);if(!s.vsans[id])return bad(`VSAN ${id} does not exist`);i.vsan=id;return ok('',true)}if(lower.startsWith('switchport mode ')){const mode=t[2]?.toUpperCase();if(!['F','E','NP'].includes(mode))return bad('Mode must be F, E, or NP');i.mode=mode;return ok('',true)}}
...
function iscsi(session,line){
  ...
  if(lower==='show state'||lower==='show running-config')return ok(JSON.stringify(s,null,2));if(lower==='show network')return ok(`VLANs\n${JSON.stringify(s.vlans,null,2)}\nInterfaces\n${JSON.stringify(s.interfaces,null,2)}`);if(lower==='show targets')return ok(JSON.stringify(s.targets,null,2));if(lower==='sessions')return ok(JSON.stringify(s.initiators.host1.sessions,null,2));if(lower==='multipath show')return ok(`Policy: ${s.multipathPolicy}\n${s.initiators.host1.sessions.flatMap(x=>x.portals.map(p=>`${x.iqn} via ${p} ${s.failedPaths.includes(p)?'failed':'active'}`)).join('\n')||'No paths'}`);
  let m=/^vlan (create|delete) (\d+)(?: (\S+))?$/i.exec(line);if(m){if(m[1].toLowerCase()==='create')s.vlans[m[2]]={name:m[3]||`vlan${m[2]}`};else{if(Object.values(s.interfaces).some(i=>String(i.vlan)===m[2]))return bad(`VLAN ${m[2]} is assigned to an interface`);delete s.vlans[m[2]]}return ok('',true)}
  ...
  m=/^interface delete (\S+)$/i.exec(line);if(m){if(m[1]==='eth0')return bad('Baseline management interface eth0 cannot be deleted');if(!s.interfaces[m[1]])return bad(`Interface ${m[1]} does not exist`);delete s.interfaces[m[1]];return ok('',true)}
  ...
  if(lower==='discover'){s.initiators.host1.discoveries=Object.keys(s.targets).filter(iqn=>s.targets[iqn].portals.length);return ok(s.initiators.host1.discoveries.join('\n')||'No targets discovered',true)}
  m=/^(login|logout) (\S+)$/i.exec(line);if(m){const target=s.targets[m[2]];if(!target||!s.initiators.host1.discoveries.includes(m[2]))return bad(`${m[2]} was not discovered`);if(m[1].toLowerCase()==='login'){if(target.chap)return bad('CHAP credentials required: login <iqn> <user> <secret>');if(!target.portals.some(p=>!s.failedPaths.includes(p)))return bad('Login failed: no active target portal');s.initiators.host1.sessions=[...s.initiators.host1.sessions.filter(x=>x.iqn!==m[2]),{iqn:m[2],portals:[...target.portals]}]}else s.initiators.host1.sessions=s.initiators.host1.sessions.filter(x=>x.iqn!==m[2]);return ok('',true)}
  m=/^login (\S+) (\S+) (\S+)$/i.exec(line);if(m){const target=s.targets[m[1]];if(!target||!s.initiators.host1.discoveries.includes(m[1]))return bad(`${m[1]} was not discovered`);if(!target.chap||target.chap.user!==m[2]||target.chap.secret!==m[3])return bad('Authentication failed');if(!target.portals.some(p=>!s.failedPaths.includes(p)))return bad('Login failed: no active target portal');s.initiators.host1.sessions=[...s.initiators.host1.sessions.filter(x=>x.iqn!==m[1]),{iqn:m[1],portals:[...target.portals]}];return ok('',true)}
  m=/^multipath policy (round-robin|failover)$/i.exec(line);if(m){s.multipathPolicy=m[1].toLowerCase();return ok(`Multipath policy set to ${s.multipathPolicy}`,true)}
  m=/^path (fail|restore) (\S+)$/i.exec(line);if(m){const known=Object.values(s.targets).some(target=>target.portals.includes(m[2]));if(!known)return bad(`Unknown portal ${m[2]}`);if(m[1].toLowerCase()==='fail')s.failedPaths=[...new Set([...s.failedPaths,m[2]])];else s.failedPaths=s.failedPaths.filter(x=>x!==m[2]);return ok('',true)}
  return bad(`Invalid command: ${line}\nType help for the supported educational command set.`);
}
...
export function executeAcademyCommand(session,input){
  ...
  const lower=line.toLowerCase();if(lower==='help'||lower==='?')return{...ok(help(session.profile)),prompt:academyPrompt(session)};if(lower.startsWith('help '))return{...ok(contextualHelp(session.profile,lower.slice(5).trim())),prompt:academyPrompt(session)};
  ...
  if(lower==='academy report'||lower==='report'){const report=academyReport(session),rows=Object.entries(report.assessments).map(([id,item])=>`${id.padEnd(18)} ${String(item.bestScore).padStart(3)}/100  ${item.bestCommands} commands`).join('\n')||'No completed incidents';return{...ok(`ACADEMY REPORT · ${report.profileName}\nCompletion: ${report.completed}/${report.total} (${report.completionPercent}%)\nAverage best score: ${report.averageScore}/100\nSession commands: ${report.commandsRun}\n${rows}`),prompt:academyPrompt(session),report}}
  if(lower==='academy export'){const report=academyReport(session);return{...ok(JSON.stringify(report,null,2)),prompt:academyPrompt(session),report}}
