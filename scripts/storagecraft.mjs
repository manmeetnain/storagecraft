...
import { ACADEMY_DEVICES, OPERATIONS_INCIDENTS, createOperationsWorkspace, loadOperationsIncident } from '../public/lib/academy-operations.js';
import { createConfigurationSafety, createCheckpoint, runConfigurationScript, rollbackCheckpoint } from '../public/lib/academy-config-safety.js';
import { COMPATIBILITY_PROFILES, ACADEMY_ROLES, createGovernedWorkspace, executeGovernedCommand, exportAuditLog } from '../public/lib/academy-governance.js';
import { CERTIFICATION_BLUEPRINT, createCertificationAttempt, recordCertificationTask, certificationReport } from '../public/lib/academy-certification.js';
...
async function academy(){const profile=String(arg('profile','brocade')).toLowerCase();if(!ACADEMY_PROFILES[profile])throw new Error(`--profile must be ${Object.keys(ACADEMY_PROFILES).join(', ')}`);header();if(process.argv.includes('--topology')){const topology=createDualFabricTopology(),health=topologyHealth(topology);console.log(`\n${paint('bold','Academy dual-fabric topology')}  ${paint('green',health.state)}`);for(const fabric of ['A','B']){console.log(`  Fabric ${fabric}  ${paint('green','UP')}  ${topology.nodes.filter(n=>n.fabric===fabric).map(n=>`${n.label} (${n.type})`).join(' → ')}`)}console.log(`  Endpoints ${paint('cyan','HOST-01 ⇄ ARRAY-01')} · redundant ${health.redundant?'yes':'no'}`);return}if(process.argv.includes('--config-safety')){const safety=createConfigurationSafety(createOperationsWorkspace());createCheckpoint(safety,'known-good');const source='[brocade-a1] switchname PROD-A1\n[brocade-b1] switchname PROD-B1';const result=runConfigurationScript(safety,source);console.log(`\n${paint('bold','Academy configuration safety')}  ${paint('green','ATOMIC PASS')}`);console.log(`  Checkpoint  ${paint('cyan','known-good')}`);console.log(`  Script      ${result.results.length} commands across two switches`);console.log(`  Diff        ${result.diff.length} desired-state changes`);result.diff.forEach(change=>console.log(`  ${paint('violet',change.path)}  ${JSON.stringify(change.before)} → ${JSON.stringify(change.after)}`));rollbackCheckpoint(safety,'known-good');console.log(`  Rollback    ${paint('green','verified')}`);return}if(process.argv.includes('--operations')){const workspace=createOperationsWorkspace(),incident=arg('incident',null);if(incident)loadOperationsIncident(workspace,String(incident));console.log(`\n${paint('bold','Academy operations workspace')}  ${paint('cyan',`${Object.keys(workspace.sessions).length} terminals`)}`);Object.entries(ACADEMY_DEVICES).forEach(([id,d])=>console.log(`  ${id.padEnd(12)} ${paint('green',d.label.padEnd(10))} ${d.profile.padEnd(8)} Fabric ${d.fabric}`));console.log(`\n${paint('bold','Advanced incidents')}`);OPERATIONS_INCIDENTS.forEach(i=>console.log(`  ${paint('red',i.id.padEnd(20))} ${i.title}`));if(incident)console.log(`\n  Loaded: ${paint('yellow',String(incident))}`);return}const session=createAcademySession(profile),one=arg('command',null);console.log(`\n${paint('bold','Storage Network Academy')}  ${paint('cyan',ACADEMY_PROFILES[profile].name)}`);console.log(paint('dim','  Safe educational simulation · no real device connections\n'));const execute=line=>{const result=executeAcademyCommand(session,line);if(result.output)console.log(result.ok?result.output:paint('red',result.output));return result};if(one!==null){console.log(`${paint('green',academyPrompt(session))} ${one}`);execute(String(one));return}if(!process.stdin.isTTY){execute('help');return}const{createInterface}=await import('node:readline');const rl=createInterface({input:process.stdin,output:process.stdout,historySize:100});const ask=()=>rl.question(`${paint('green',academyPrompt(session))} `,line=>{if(['quit','logout','exit academy'].includes(line.trim().toLowerCase()))return rl.close();execute(line);ask()});rl.on('close',()=>console.log(paint('dim','\nAcademy session ended; no real infrastructure was changed.')));ask()}
function academyGovernance(){header();const governed=createGovernedWorkspace(createOperationsWorkspace(),{actor:'cli-auditor',role:'observer'});executeGovernedCommand(governed,'switchshow');executeGovernedCommand(governed,'switchname denied');const audit=exportAuditLog(governed);console.log(`\n${paint('bold','Academy governance')}  ${paint('green',audit.verified?'AUDIT VERIFIED':'AUDIT INVALID')}`);console.log(`  Roles     ${Object.keys(ACADEMY_ROLES).join(' · ')}`);Object.values(COMPATIBILITY_PROFILES).forEach(item=>console.log(`  Profile   ${paint('cyan',item.label)} · ${item.version}`));audit.events.forEach(event=>console.log(`  ${event.sequence} ${paint(event.decision==='ALLOW'?'green':'red',event.decision)} ${event.actor}/${event.role} ${event.deviceId} ${event.hash}`));}
function academyCertification(){header();const attempt=createCertificationAttempt(String(arg('candidate','CLI Learner')),'cli-demonstration');CERTIFICATION_BLUEPRINT.forEach(task=>recordCertificationTask(attempt,task.id,88,`${task.id} verified CLI demonstration evidence`));const report=certificationReport(attempt,{generatedAt:'deterministic-cli-demo',auditVerified:true,compatibilityProfiles:Object.values(COMPATIBILITY_PROFILES).map(item=>item.id)});console.log(`\n${paint('bold','Academy practical certification')}  ${paint('green',report.summary.status)}`);console.log(`  Candidate       ${paint('cyan',report.candidate)}`);console.log(`  Weighted score  ${paint('green',`${report.summary.weightedScore}/100`)}`);console.log(`  Domains         ${report.summary.completed}/${report.summary.total} complete · minimum ${report.summary.minimumDomain}/100`);console.log(`  Issuer          ${report.issuer}`);}
function kv() {
...
try { if(command==='raid') raid(); else if(command==='erasure') erasure(); else if(command==='write-path') writePath(); else if(command==='lsm') lsm(); else if(command==='gpu-plan') gpuPlan(); else if(command==='rag-size') ragSize(); else if(command==='ai-path') aiPath(); else if(command==='nvme-queues') nvmeQueues(); else if(command==='san-failure') sanFailure(); else if(command==='academy'&&process.argv.includes('--certification')) academyCertification(); else if(command==='academy'&&process.argv.includes('--governance')) academyGovernance(); else if(command==='academy') await academy(); else if(command==='kv') kv(); else if(command==='topics') topics(); else if(command==='doctor') await doctor(); else help(); }
